...
class EmployeeApp:
    ...
    def display_data(self, filtered_rows=None):
        ...
        query = f'''
        SELECT NationalCode, FirstName, LastName, BirthDate,GenderID,MaritalStatusID, Mobile 
        FROM [PersonalInfo]
...
class SingleInstance:
    def __init__(self, port=65432):
        ...
        try:
            # سعی در اتصال به پورت، اگر موفق نشدیم یعنی برنامه قبلاً باز است
            self.sock.bind(("127.0.0.1", self.port))
        except socket.error:
            print("این برنامه در حال حاضر اجرا می‌شود.")
            sys.exit()
